...
func NewRLYSMS(accountSid string,authorToken string,appId string) *RLYSMS{
...
// 发送的消息与手机号码个数要匹配
func (this *RLYSMS)mSGJSON(message string, duration int, mobiles ...string) string {
	/*
	{"to":"13911281234,15010151234,13811431234","appId":
 "ff8080813fc70a7b013fc72312324213","templateId":"1","datas":["替换内容","替换内容"]}
	*/
	tDuration:=strconv.Quote(strconv.Itoa(duration))
	body :=map[string]string{
		"to":strings.Join(mobiles,","),
		"appId":this.appId,
		"templateId":this.templateId,
		"datas":"["+strconv.Quote(message)+","+tDuration+"]",
	}
...
func (this *RLYSMS)SendMsMs(message string,duration int, mobiles ...string)(rsp *http.Response,err error){
	if this.flag==false{
	...
	smsUrl := this.baseUrl+"2013-12-26/"+"Accounts/"+this.accountSid+"/SMS/TemplateSMS?sig="+sig

	content:=this.mSGJSON(message,duration,mobiles...)
	// 构造请求体并添加内容
